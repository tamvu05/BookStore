...
class ImportReceiptFormModal {
    ...
    resetModal() {
        // Reset form inputs
        this.inputDate.value = new Date().toISOString().substring(0, 10)
...
class ImportReceiptTable {
    ...
    async exportExcel() {
        try {
            ...
            let filename = 'DanhSachPhieuNhap.xlsx'
...
class DetailModal {
    ...
    async initValue(id) {
        try {
            ...
            this.labelDate.textContent = receipt.NgayNhap
            ...
            details.forEach(detail => {
                ...
                html += `
                    <tr>
                        ...
                        <td class="text-end">${detail.DonGiaNhap.toLocaleString('vi-VN')}</td>
